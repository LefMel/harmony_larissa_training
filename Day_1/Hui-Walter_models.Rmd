...
rmarkdown::render('Hui-Walter_models.Rmd', 'beamer_presentation', params=list(presentation=TRUE))
# And for html:
rmarkdown::render('Hui-Walter_models.Rmd', 'html_document', params=list(presentation=FALSE))
```
...
- Reference: Hui, S. L., & Walter, S. D. (1980). Estimating the error rates of diagnostic tests. Biometrics, 167-171.


## Hui-Walter Model (II)

- The Hui-Walter model is also known as the two_test - two_population setting

- We will start with the two_test - one_population setting (`hw_definition`) and continue adding both tests and populations
...
we will come back with some exercises on the hw_definition model
...
What happens if you set an initial value for a parameter outside of the parameter space?
...
- Parameters (par) to be estimated
  - S * (2R + 1) ~= 2R + S

- Degrees of freedom (df) offered by the data
  - S*(2^R - 1)
